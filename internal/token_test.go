...
func TestRetrieveToken_InParams(t *testing.T) {
	...
	defer ts.Close()
	_, err := RetrieveToken(context.Background(), clientID, "", ts.URL, url.Values{}, AuthStyleInParams, "POST")
	if err != nil {
...
func TestRetrieveTokenWithContexts(t *testing.T) {
	...
	_, err := RetrieveToken(context.Background(), clientID, "", ts.URL, url.Values{}, AuthStyleUnknown, "POST")
	if err != nil {
	...
	cancel()
	_, err = RetrieveToken(ctx, clientID, "", cancellingts.URL, url.Values{}, AuthStyleUnknown, "POST")
	close(retrieved)
	if err == nil {
